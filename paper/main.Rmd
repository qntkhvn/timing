...
abstract: |
  ...
  |
  | *Keywords:* Bayesian statistics, multilevel model, uncertainty quantification, tracking data, American football
header-includes:
...
\captionsetup{font={stretch=1.45}}
...
To make these fine-grained data accessible and promote public research, the NFL has organized an annual event called the Big Data Bowl since 2018.
...
@chu2020route detect the types of route ran by receivers via a model-based clustering approach.
...
This task was challenging previously, as position groups such as defensive linemen simply lacked reliable metrics to grade their performance prior to the availability of high-resolution data.
...
@yurko2024nfl evaluates defensive pass coverage by comparing the defender positioning at the moment of catch to a baseline hypothetical player.
...
This opens up an opportunity to leverage pre-snap information to better understand what each team and player will do during a play post-snap.
...
This can be considered as a quarterback (QB) skill, since the QB is responsible for controlling the cadence and ensuring the offense operates smoothly in every play.
...
* For the remaining plays (specifically, plays where motion player at snap is not the only player in motion since line set, and plays with players charted as in motion at snap but without a `man_in_motion` event):
    ...
    * Using plays with $t_i^{\text{motion}}$ identified from earlier, we first observe the distribution for the ratio between the motion player's speed at `man_in_motion` and their maximum speed between line set and ball snap (see Figure \ref{fig:fig_ratio_distribution}).
...
[^inconsistency]: See https://www.kaggle.com/competitions/nfl-big-data-bowl-2025/discussion/543709.

For context, Figure \ref{fig:fig_timing_distribution} displays the distribution for the snap timing across the considered plays.
...
The observed timing between motion and snap on a play is likely attributable to numerous variables---from contextual and spatiotemporal features, to the players and teams involved.
...
This information, unlike the other variables, is not provided in the data, prompting us to consider a clustering algorithm to identify the different types of motion.
...
We fit a Gaussian mixture model to detect the types of motion by NFL receivers using the `mclust` package in `R` [@scrucca2023model; @r2024language], which performs maximum likelihood estimation via the EM algorithm.
We use the Bayesian information criterion (BIC) to select the optimal model and number of Gaussian clusters $G^*$ [@raftery2006variable].
...
We further validate our results through visual inspection of motion trajectories of players in each cluster.
Figure \ref{fig:fig_motion_clusters} displays sample paths of motion players in each of the 6 cluster from the starting point of motion (in red) to the ball snap (in blue).
We see clear structural differences in the player paths between the cluster assignments, which can then be interpreted to add more football context to the results.
...
We emphasize that our motion identification approach is only a starting point, and future improvements regarding feature engineering and unsupervised learning algorithm may produce more reasonable clustering results.

<!-- To add more context, we interpret our cluster assignments as follows. -->
...
We perform diagnostics by inspecting trace plots as well as computing the $\widehat{R}$ [@gelman1992inference; @brooks1998general] and effective sample size statistics [@gelman2013bayesian], and find no issues with model convergence and efficiency.
...
Table \ref{tab:fixedeff} summarizes the results for the fixed effects terms of our multilevel model at the mean parameter level.
...
This could be due to more complex offensive play calls on later downs, or the defense's tendency to disguise their coverages more on crucial downs, requiring the quarterback to make last-second reads and adjustments.
...
\begin{table}[tbp]
\caption{Posterior estimates for the fixed effects coefficients (at the mean level) for the multilevel model for snap timing. Note that the reference down level is first down, denoted down:1; the reference number of timeouts remaining level is 0, denoted timeouts:0; the reference motion player's position level is running back (RB), denoted position:RB; and the reference motion type level (obtained via clustering) is the first motion cluster label, denoted motion:1. \label{tab:fixedeff}}
\centering
\begin{tabular}{lrrrr}
\hline
...
As it turns out, among the considered random effects, the largest source variation is between quarterbacks when modeling the snap timing shape, as captured by $\hat \tau_q = 0.297$.
**TODO: what else should be written here.**
...
As a side note, the posterior mean for the shape random effect scarcely correlates with the rate of motion on all passing plays $(r=0.11)$ during the first nine weeks of the 2022 season.
Over this period, there is also a weak correlation between the motion rate and havoc rate across all passing plays $(r=-0.09)$.
...
Since our model is fit in a Bayesian framework, it allows us to quantify uncertainty with posterior distributions for all of the parameters of interest.
Our result suggests that higher variability in snap timing is beneficial for the passing game, as it relates to experiencing less disruptions generated by the defense.
...
Further, despite the reasonable set of inputs in our cluster analysis, there is possibly room for improvement in identifying the motion types by having more refined contextual and tracking features.
Alternatively, we could consider a functional clustering approach, such as the implementation in @chu2020route for route identification.
...
We leave the exploration of these challenging issues for the future.

**[[more discussions???]]**

<!-- For instance, the success of the Kansas City Chiefs offense in 2022 is not solely due to Patrick Mahomes, but also because of the brilliant offensive mind of head coach Andy Reid. -->
<!-- The starting center, Creed Humphrey, who was one of the top centers in 2022 (named Pro Bowl and second-team All-Pro), also plays a major part in the team's snap timing execution. -->
