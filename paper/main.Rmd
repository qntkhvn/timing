...
We use weakly informative prior distributions for the standard deviation parameters in our model, which are as follows.
$$
\begin{aligned}
...
We find good evidence of convergence after visual inspection of trace plots, as also supported by all $\widehat{R}$ values being very close to 1.
...
Our clustering input contains tracking data features describing the motion player's location at different on-field events relative to where the center lines up before the snap.
...
We emphasize that our motion identification approach is only a starting point and should serve as a foundation for future work.
...
For the model of mean snap timing parameter $\mu$, we observe the highest posterior mean for the standard deviation of motion players ($\hat \sigma_m = 0.151$), followed by quarterbacks ($\hat \sigma_q = 0.093$) and defenses ($\hat \sigma_d = 0.029$).
Note that the standard deviation estimates for the two offensive groups are farther away from zero than the defensive team's, suggesting variability between players within each position group.

For both motion players and quarterbacks, we provide analyses of their random effects $b_q$ and $b_m$ at the mean level in Section \ref{sec:model-supp} of the Supplementary Materials, with Figures \ref{fig:fig_receiver_posterior_distributions} and \ref{fig:fig_qb_mean_posterior_distributions} showing their respective posterior distributions.
More importantly, among all considered random effects, the largest source variation is between quarterbacks when modeling the snap timing shape $\alpha$, as captured by $\hat \tau_q = 0.297$.
...
We highlight that our framework is modular, so that our simple algorithm in Section \ref{sec:clustering} can be replaced by other approaches for clustering the type of motion.
...
For instance, the success on offense of the 2022 Kansas City Chiefs (the year in which they won the Super Bowl) is not solely due to Patrick Mahomes, but also because of the brilliant offensive mind of head coach Andy Reid.
...
We leave the exploration of these challenging issues for future work.
...
We visually inspect and analyze the results of fitting the Gaussian mixture model in Section \ref{sec:clustering} for clustering the type of receiver motion.
...
## Additional multievel model results {#sec:model-supp}

Figure \ref{fig:fig_receiver_posterior_distributions}

```{r}
#| label: fig_receiver_posterior_distributions
#| fig-cap: "Posterior distributions of the motion player random effect (when modeling the mean snap timing). Results shown here are for receivers with at least 20 times going in motion on plays with receivers in motion at snap and running a route. For each player, the posterior mean point estimate and corresponding credible intervals are depicted."
#| out-width: 70%
knitr::include_graphics("../figures/receiver_posteriors_slab.png")
```

Figure \ref{fig:fig_qb_mean_posterior_distributions}



```{r}
#| label: fig_qb_mean_posterior_distributions
#| fig-cap: "Posterior distributions of the quarterback random effect (when modeling the mean snap timing). Results shown here are for quarterbacks with at least 50 pass attempts on plays with receivers in motion at snap and running a route. For each player, the posterior mean point estimate and corresponding credible intervals are depicted."
#| out-width: 70%
