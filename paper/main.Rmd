...
Our clustering input contains tracking data features describing the receiver locations at different on-field events relative to where the center lines up before the snap.
...
As mentioned in Section \ref{sec:model}, we now offer analyses of the random effects in the mean parameter model for the two group on offense: quarterbacks and motion players.
...
The scatterplot reveals no inherent relationship between the average and variability snap timing, and the QB ranking in Figure \ref{fig:fig_qb_mean_posterior_distributions} also does not agree with the QB snap timing leaderboard in Section \ref{sec:leaderboard}.
Hence, a quarterback can be consistent or variable with managing snap timing, this does not necessarily depend on the duration of snap timing itself on average.
...
We notice that Stefon Diggs and Tyreek Hill are the top two receivers in terms of their posterior mean values.
These are two of the most elite receivers in 2022, with Tyreek Hill especially being known for his lightning-quick running speed and was part of the electric 2022 Miami Dolphins offense.
Thus, at first glance, one could suspect that these estimates are indicative of receiver quickness and agility to some degree.
...
#| fig-cap: "Posterior distributions of the quarterback random effect when modeling the mean snap timing. Results shown here are for NFL quarterbacks with at least 50 pass attempts on plays with receivers in motion at snap and running a route. For each player, the posterior mean point estimate and corresponding credible intervals are depicted."
#| out-width: 75%
...
#| fig-cap: "Posterior distributions of the motion player random effect when modeling the mean snap timing. Results shown here are for NFL receivers with at least 20 times going in motion on plays with receivers in motion at snap and running a route. For each player, the posterior mean point estimate and corresponding credible intervals are depicted."
#| out-width: 75%
