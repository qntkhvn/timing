...
Consider the following two passing plays by Chiefs QB Patrick Mahomes in the 2022 NFL regular season.
The first play was a short pass to M. Valdes-Scantling for a first down against the Colts in week 3.
The second play was a TD pass to J.Smith-Schuster against the 49ers in week 7.

[[TODO: add more about these plays]]

In both of these plays, KC uses the same type of motion---glide, as the motion player (highlighted in gold) lines up in an outside wide alignment, and then starts motioning towards the inside, and the ball is snapped when the player is about halfway to the center.

But these two plays differ in the time between start of motion and ball snap.
In the first play, the snap happens 3.6 seconds after the motion begins, whereas for the second, the snap occurs within a half second of the motion.

In this work, we focus on evaluating a passer's ability to react and adapt to different variability in timing.
...
This information, however, is not included in the original data, prompting us to consider a Gaussian mixture model to cluster the motion types.
...
[[TODO: mention limitations in discussion - DONE]]
