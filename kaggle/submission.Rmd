...
which is well-suited for modeling a positive, "waiting time" variable like our case.

Of primary interest, we model the shape $\alpha$ of the distribution with QB random effects (which is proportional to the Gamma distribution variance).
...
These include the current down, number of timeouts remaining for the offense, play clock at the start of motion, motion player's position (WR, TE, RB/FB), number of players in motion since line-set, and type of motion, as they all can impact the timing between motion and snap.
The type of motion is a vital feature for modeling snap timing, since it relates to play design.
...
We choose the optimal number of Gaussian clusters to be X based on BIC, and validate the cluster assignment through visual inspection of player motion trajectories in each cluster. We condition on these X cluster labels as fixed effects in modeling the mean snap timing parameter $\mu$, so that that our estimates of QB snap timing variability $u_q$ are not confounded by different types of motion.

[[TODO: insert the number of clusters in the paragraph above as X]]
...
We quantify uncertainty for our model parameters with posterior distributions. Ultimately, among the considered random effects, the largest source variation is between QBs when modeling the snap timing shape (captured by $\tau_q^2$). Thus, we focus on the QB shape random effects $u_q$ for the following analysis.
...
Note that there is considerable uncertainty in our estimates, as demonstrated by the rather wide credible intervals (CI), which is unsurprising given our limited sample of motion plays.
...
Next, we examine the relationship between the posterior mean for the QB shape random effect $u_q$ and a measure of "havoc rate".
...
The figure below is a scatterplot of the QB shape random effect and havoc rate for all pass plays over the first nine weeks of the 2022 NFL season (left) and only the considered motion plays (right) for the same subset of QBs as before.
...
As a side note, the posterior mean for the shape random effect scarcely correlates with the rate of motion on all pass plays ($r = 0.08$).
...
We demonstrate that higher variability in snap timing is beneficial for the passing game, as it relates to experiencing less disruptions generated by the defense. Although we solely focused on snap timing for passing plays with motion, we believe our estimates may also serve as proxies of QB awareness or pocket presence as indicated by the moderate correlation with havoc rate across all passing plays (not just the plays considered in our modeling framework).
...
Since neither of these tasks is the main focus of our work, we leave these explorations for the future. Additionally, we limited our analysis to only passing plays with receivers in motion at snap. This introduces a selection bias into our study by excluding running plays and various QBs (e.g., Jalen Hurts and Joe Burrow). We also recognize that while we refer to our estimates as "QB random effects", they are certainly correlated with the team's center and coach's play calling. Nevertheless, __we believe our contribution is a novel description of snap timing variability and reveals insight into an understudied area of football.__
