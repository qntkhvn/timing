...
which is well-suited for modeling a positive, continuous ("waiting time") variable like our case.
...
We select the optimal number of Gaussian clusters to be $G=10$ using BIC, and validate the cluster assignment through visual inspection of player motion trajectories in each cluster.
...
For instance, despite his pay, Daniel Jones' 95% CI does not overlap at all with the top five quarterbacks' 95% CIs.
...
As a side note, the posterior mean for the shape random effect scarcely correlates with the rate of motion on all passing plays $(r = 0.08)$ over the first nine weeks of the 2022 season.
