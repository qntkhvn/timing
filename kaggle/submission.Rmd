...
Ultimately, we obtain posterior distributions for our model parameters (estimated using MCMC) and perform the following analysis, **focusing on the shape/variance parameter**.
...
For each player, the posterior mean of the shape/variance provides an estimate for their ability to maintain variability in snap timing, **with higher posterior mean values corresponding to greater variability in timing between motion and snap**.
...
Note that there is considerable uncertainty in our estimate, as indicated by the somewhat wide credible intervals, which is unsurprising given our limited sample of motion plays.
...
Variability in snap timing is an essential aspect for an offense to dictate plays and make it difficult for the defense to anticipate and react.
...
Since either of these task is the main focus of our work, we leave these explorations for the future.
