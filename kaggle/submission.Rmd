...
In each animation, the solid gold line along the motion player's path represents the trajectory portion between the start of motion and the ball-snap.
...
\\
\log \mu_i &= \alpha_0 + \boldsymbol{\beta X_i} + b_{q[i]} + b_{m_[i]} + b_{d[i]} \\
b_q  &\sim \textsf{Normal}(0, \sigma^2_q)\\
b_m  &\sim \textsf{Normal}(0, \sigma^2_m)\\
b_d  &\sim \textsf{Normal}(0, \sigma^2_d)\\
\\
\log \phi_i &= \gamma_0 + u_{q[i]} \\
u_q  &\sim \textsf{Normal}(0, \tau^2_q)\\
\end{aligned}
$$


In detail, we model the response $\delta_i > 0$ (snap timing for play $i$) using a Gamma distribution (with log link function), which is well-suited for a positive, continuous variable like our case.
...
From an offensive standpoint, we hypothesize that a higher timing variability is beneficial because it prevents defenses from predicting when the ball will be snapped.
This also allows the offense to control the game flow and exploit defensive vulnerabilities.
<!-- insert shred it gruden gif https://x.com/BarstoolGruden/status/1858193718823522678 -->
...
In particular, we compute the displacement in both sideline and endzone directions between the motion player at ball-snap and the snapper.
We also include the displacement with respect to the sideline between the snapper and motion player at the start of motion as well as when they cross the line of scrimmage (LOS).
...
Further, despite the reasonable set of inputs in our proposed clustering algorithm, there is certainly room for improvement in identifying the motion types by having more refined features, or alternatively using a functional clustering approach.
